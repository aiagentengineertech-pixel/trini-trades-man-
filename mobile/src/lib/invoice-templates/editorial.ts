// "Editorial" invoice template — elegant navy + blush pink.
// Pure (settings, draft) => standalone HTML document for expo-print → PDF.
import {
  InvoiceSettings,
  InvoiceDraft,
  money,
...
// Tiny inline SVG-ish glyph using a colored dot (no external resources).
function iconLine(text: string): string {
  return (
    `<div class="contact"><span class="dot"></span><span>${esc(text)}</span></div>`
  );
}
...
export function render(s: InvoiceSettings, d: InvoiceDraft): string {
  ...
  const metaHtml = metaRows
    .map(
      ([k, v]) =>
        `<tr><td class="meta-k">${esc(k)}</td><td class="meta-v">${esc(v)}</td></tr>`
    )
    .join('');
  ...
  const taglineHtml = s.tagline
    ...
    : '';
  const contactHtml = contacts(s).map(iconLine).join('');
  ...
  const itemRows = d.lines
    .map((l) => {
      const detail = l.detail
        ? `<div class="it-detail">${esc(l.detail)}</div>`
        : '';
  ...
  totalRows.push(
    `<tr><td class="t-k">SUBTOTAL</td><td class="t-v">${money(totals.subtotal)}</td></tr>`
  );
  if (totals.discount > 0) {
    totalRows.push(
      `<tr><td class="t-k">DISCOUNT</td><td class="t-v">-${money(totals.discount)}</td></tr>`
    );
  }
  if ((d.taxPct || 0) > 0) {
    const taxLabel = d.taxPct === 12.5 ? 'VAT (12.5%)' : `TAX (${d.taxPct}%)`;
    totalRows.push(
      `<tr><td class="t-k">${esc(taxLabel)}</td><td class="t-v">${money(totals.tax)}</td></tr>`
    );
  }
  ...
  const signatureHtml = s.signatureName
    ? `<div class="sign">` +
      ...
      (s.signatureTitle
        ? `<div class="sign-title">${esc(s.signatureTitle)}</div>`
        : '') +
      `</div>`
    : '';
  ...
  const paymentBlock = hasPayment
    ? `<div class="pay">` +
      `<div class="pay-head">PAYMENT INFORMATION</div>` +
      bankHtml +
      extraHtml +
      `</div>`
    : '';
  ...
  // ---- Footer note ----
  const footerNoteHtml = s.footerNote
    ? `<div class="foot-note">${multiline(s.footerNote)}</div>`
    : '';
...
<style>
  * { box-sizing: border-box; }
  ...
  /* soft blush brush blob behind the lower table area */
  .blob {
    position: absolute;
    left: -10%;
    right: -10%;
    top: 360px;
    height: 420px;
    background: radial-gradient(60% 70% at 50% 40%, ${PALE} 0%, rgba(246,228,228,0.55) 45%, rgba(246,228,228,0) 72%);
    z-index: 0;
  }
  ...
  .totals tr.total-due .t-k {
    color: ${NAVY};
    font-size: 12px;
  }
  .totals tr.total-due .t-v {
    color: ${NAVY};
    font-family: ${SERIF};
    font-size: 22px;
    font-weight: 700;
  ...
  /* ===== Signature ===== */
  .sign { margin: 6px 0 24px; max-width: 240px; }
  .sign-name {
    font-family: ${CURSIVE};
    color: ${NAVY};
    font-size: 30px;
  ...
  .pay-k { color: ${GREY}; }
  .pay-v { color: ${INK}; font-weight: 600; }
  ...
  <div class="page">
    ...
    <div class="totals-wrap">
      <table class="totals"><tbody>
        ${totalsHtml}
        <tr class="total-due"><td class="t-k">TOTAL DUE</td><td class="t-v">${money(
          totals.total
        )}</td></tr>
      </tbody></table>
    </div>
    ...
    <div class="appreciate">We Appreciate You</div>
    ${footerNoteHtml}
  </div>
</body>
</html>`;
